...
- ✅ **多用户模式（可选）**：开启后可注册/登录账号，每个账号独立保存 Token、查询自己的余额，首个注册账号为管理员
...
services:
  token-manager:
    ...
    environment:
      ...
      # - MULTI_USER=true
      # 访问码：多用户模式下作为注册邀请码；普通模式下打开界面和所有API都要求先输入
      - ACCESS_CODE=
...
| 数据目录 | `./data` | 密钥文件保存在这里，务必保留这个挂载，否则容器重建后密钥丢失 |
| DATA_DIR | `/data` | 容器内数据目录，配合上面的挂载使用，一般无需修改 |
| ACCESS_CODE | 空 | **访问码**。普通模式下打开界面和所有 API 都要求先输入访问码；多用户模式下变为**注册邀请码**（注册时必须输入，登录不用）；留空则不启用/开放注册 |
| MULTI_USER | 关 | 设为 `true` 启用**多用户模式**（注册/登录，每个账号独立保存 Token，首个注册账号为管理员），详见下方"多用户模式" |
...
默认关闭。在 docker-compose.yml 里加上 `- MULTI_USER=true`（并 `docker compose up -d`）启用后，界面变为**注册 / 登录**，每个账号拥有完全独立的 Token 存储、自定义服务商和余额查询，互不可见。

- **注册**：在登录页切换到"注册"标签创建账号。`ACCESS_CODE` 此时变为**注册邀请码**——设置了则注册必须输入（防止知道地址的陌生人自行注册），留空则开放注册；登录永远不需要邀请码
- **管理员**：**第一个注册的账号自动成为管理员**，界面右上角用户菜单里多出"用户管理"：查看用户列表、添加用户、重置密码、删除用户（不能删除自己和最后一个管理员）
- **数据隔离与安全**：密码经 PBKDF2 哈希存储；各账号数据保存在 `data/users/<用户名>/` 下；修改密码后该账号所有旧登录立即失效
- **旧数据迁移**：启用多用户后，原单用户模式保存在 `data/` 根目录的密钥，会在**首个账号注册时自动迁移**给它（即首个账号 = 管理员 = 原有数据的继承人）
- **会话有效期**：与访问码模式共用 `SESSION_DAYS`（默认 30 天），右上角用户菜单可随时"退出登录"或"修改密码"

**典型配置**（家人朋友各自账号 + 防陌生人注册）：

```yaml
      - MULTI_USER=true
      - ACCESS_CODE=你和家人约定的邀请码
```

**关闭多用户**：删掉 `MULTI_USER` 那行并 `docker compose up -d`，回到访问码/无鉴权模式。各账号数据保留在 `data/users/` 下不会丢失，但界面不再出现登录入口。
